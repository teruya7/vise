...
"""Element colors for VESTA visualization.

This module provides RGB color mappings for chemical elements used
in VESTA crystal structure visualization.

Originally developed by Naoki Tsunoda.
"""

from typing import Dict, Tuple

# RGB color values for each element (normalized to 0-1 range)
ATOM_COLORS: Dict[str, Tuple[float, float, float]] = {
    ...
    "XX": (0.30000, 0.30000, 0.30000),  # Dummy species
}


def atom_color(element: str) -> str:
    """Get RGB color string for VESTA (0-255 range, space-separated).

    Args:
        element: Element symbol.

    Returns:
        Space-separated RGB color string (e.g., "255 0 0" for red).

    Examples:
        >>> atom_color("O")
        '255 3 0'
    """
    return " ".join(str(int(x * 255)) for x in ATOM_COLORS[element])


# Backward compatibility alias
atom_property = ATOM_COLORS
